...
def pairs(row):
    ...
    return [(row[i], row[j]) for i in range(length) for j in range(i + 1, length)]
...
class TestChecksum(TestCase):
    def test_checksum_part1(self):
        ...
        2 4 6 8
        """
        self.assertEqual(18, checksum_part1(sample))

        print('Checksum part 1: %s' % checksum_part1(puzzle))
    ...
    def test_checksum_part2(self):
        ...
        self.assertEqual(9, checksum_part2(sample))

        print('Checksum part 2: %s' % checksum_part2(puzzle))